...
class UserRegisterUseCase:
    ...
    def execute(self, request, params: params.UserRegister) -> user_models.User:
        if user_models.User.objects.filter(
            Q(username=params.username) | Q(email=params.email)
        ).exists():
...
class RegistrationStep2PhoneNumberUseCase:
    ...
    def execute(
        self, request, params: params.RegistrationPhoneNumberParams
    ) -> user_models.User:
        try:
            user_temp_data = user_models.UserTempData.objects.get(
                otp=params.otp, target_value=request.session.get("phone_number")
            )
