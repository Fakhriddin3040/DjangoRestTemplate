...
class RegistrationStep2EmailUseCase:
    ...
    def execute(
        self, request, params: params.RegistrationEmailParams
    ) -> user_models.User:
        try:
            user_temp_data = user_models.UserTempData.objects.get(
                otp=params.otp, target_value=request.session.get("email")
            )
...
class UserEmailRegisterUseCase:
    ...
    def execute(self, request, params: params.UserRegister) -> user_models.User:
        ...
        if user_models.User.objects.filter(
            Q(username=params.username) | Q(phone_number=params.phone_number)
        ).exists():
