...
class RegistrationOAuthUseCase:
    ...
    def execute(self, request, params: params.RegistrationOAuthParams) -> user_models.User:
        token = params.token
        
        try:
            decoded_token = decode_jwt(token=token, algoritms=["RS256"], options={"verify_signature": False})
            email = decoded_token.get('email')
            name = decoded_token.get('name')
            user_params = UserRegister(email=email, 
                                                username=name, 
